...
onready var _rigged_character: MeshInstance = $Armature/Skeleton/RiggedMesh
...
##########################
## I may need to actually generate the
## animationplayer node so that
## it starts with the correct parent applied
## or I can just change its root node to whichever
## parent it has onready


func _ready():
	_animation_player.root_node = self.get_name()
