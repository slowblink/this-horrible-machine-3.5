...
func _on_enter(_args) -> void:
	...
	target.skin.position.y = - 2
	target.skin.scale.x = - target.wall_dir
	target.skin.position.x = - target.wall_dir * 4
	target.skin.rotation = - target.wall_dir * PI/2


func _on_update(_delta) -> void:
	var dir_out_of_wall := false
	if target.wall_dir < 0 and Input.is_action_just_pressed("right") \
			or target.wall_dir > 0 and Input.is_action_just_pressed("left"):
		dir_out_of_wall = true
	
	if dir_out_of_wall or Input.is_action_pressed("crouch"):
